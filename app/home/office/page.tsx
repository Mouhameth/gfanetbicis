...
const Home = () => {
    ...
    const { data: result, isLoading, error, mutate } = useSWR(`${url}`, () => axiosAuth.post<Stats>(url, JSON.stringify({ date: now.toLocaleDateString('fr-FR'), ids: [session?.user.office_id] })).then((res) => res.data));
    ...
    useEffect(() => {
        console.log(session?.user.office_id);
        
        useChangeTitle.onChanged("Rapport global du flux des clients en agence");
        if (session && session.user && session.user.office_id) {
            setOfficeName(session.user.office?.name || '');
        }
    ...
    const handleFilter = async (date: string) => {
        ...
        try {
            ...
            const res = await axiosAuth.post<Stats>(url, JSON.stringify({ date: date, ids: selectedOffices.length === 0 ? [session?.user.office_id] : selectedOffices }));
            if (res.status == 200) {
    ...
    const handleWeekFilter = async (start: string, end: string) => {
        ...
        try {
            ...
            const res = await axiosAuth.post<Stats>(`${url}/week`, JSON.stringify({ start: start, end: end, ids: selectedOffices.length === 0 ? [session?.user.office_id] : selectedOffices }));
            if (res.status == 200) {
                setFilterStats(res.data);
            }
    ...
    if (isLoading || loading || !result || timeLoading) {
        return <Loader />
    }
